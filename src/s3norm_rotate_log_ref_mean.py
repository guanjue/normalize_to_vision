...
################################################################################################
###
def s3norm(sample_num, sig1_wg_raw, sig2_wg_raw, upperlim, lowerlim):
	sig1_output_name = sig1_wg_raw+'.'+'s3norm'
	sig2_output_name = sig2_wg_raw.split('.')[0]+'.'+'s3norm'
	...
	write2d_array(sig2_norm, sig2_output_name + '.ref.txt')
	...
	plt.xlabel(sig2_output_name + '.s3norm')
	plt.ylabel(sig1_output_name + '.s3norm')
...
def main(argv):
	try:
		...
		print 'time python peaknorm_rotate_log_ref_mean.py -n sample_num -a reference_dataset -b target_dataset -u upperlim -l lowerlim'
		sys.exit(2)
	...
	s3norm(sample_num, sig1_wg_raw, sig2_wg_raw, upperlim, lowerlim)

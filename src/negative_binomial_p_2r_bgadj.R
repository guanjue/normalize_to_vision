...
### read data
sig = read.table(paste(signal_track_file, sep=''), header = F)
if (input_track_file==1){
	input = as.matrix(rep(1, dim(sig)[1]))
} else{
	input = read.table(paste(input_track_file, sep=''), header = F)
}

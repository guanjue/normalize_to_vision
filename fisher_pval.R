...
### write output
write.table(fisher_p, paste(input_folder, cell_marker, '.fisher_p.txt', sep=''), quote=FALSE, col.names=FALSE, row.names=FALSE, sep='\t')
...
write.table(c(FRiP, SNR), paste(input_folder, cell_marker, '.fisher_p.frip_snr.txt', sep=''), quote=FALSE, col.names=FALSE, row.names=FALSE, sep='\t')
